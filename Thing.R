...
# Step 1: Calculate win ratios for all players
in_deg <- igraph::degree(chess_net, mode = "in")   # losses
out_deg <- igraph::degree(chess_net, mode = "out") # wins
win_ratios <- out_deg / (in_deg + out_deg + 0.001)  # avoid division by 0
total_wins <- out_deg
...
# Create directed edge list preserving white->black direction
class(chess_edges_directed)


chess_ergm_net <- network(as.matrix(chess_netw[,1:2]), directed = TRUE)


set.vertex.attribute(chess_ergm_net, "win_ratio", win_ratios)
...
# Step 1: Calculate network centrality measures
player_centrality <- data.frame(
  player_id = V(chess_net)$name,
  degree = igraph::degree(chess_net),
  betweenness = igraph::betweenness(chess_net, normalized = TRUE),
  closeness = igraph::closeness(chess_net)
)

# Step 2: Calculate performance measures
in_deg <- igraph::degree(chess_net, mode = "in")   # losses
out_deg <- igraph::degree(chess_net, mode = "out") # wins
total_games <- in_deg + out_deg

player_centrality$total_wins <- out_deg
player_centrality$win_ratio <- out_deg / (total_games + 0.01)  # avoid division by 0

# Step 3: Test if centrality predicts performance
model_wins <- lm(total_wins ~ degree + betweenness + closeness, 
                 data = player_centrality)

model_ratio <- lm(win_ratio ~ degree + betweenness + closeness, 
                  data = player_centrality)
